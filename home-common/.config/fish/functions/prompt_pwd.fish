function prompt_pwd --description 'Print the current working directory, shortened to fit the prompt'
    ...
    set -q __fish_prompt_ellipsis; or set -l __fish_prompt_ellipsis …
    ...
    if [ $fish_prompt_pwd_dir_length -eq 0 ]
        ...
        if [ (count $all) -gt 1 ]
            ...
            for part in $all
                ...
                if [ $reduction -gt 0 ]
                    ...
                    set parts $parts (string sub -l (math "$fish_prompt_pwd_dir_length-1") $part)"…"
                else
        ...
        if test $num_parts -gt $__fish_prompt_max_dirs
         set parts $__fish_prompt_ellipsis $parts[(math 0 - $__fish_prompt_max_dirs)..-1]
        end
        string join "/" $parts
    end
end
